...
class Mcts_plus(object):
    ...
    def __init__(self, board, policy_value_function, r, color = 'X', is_selfplay = 0):
        self.color = color
    ...
    def mcts_run(self):  # 若为0 为对战模式，传输action为最大概率
        ...
        root.next_locations = list(root.board.get_legal_actions(root.color))
        self.simulation(root)
        expand_node = self.expand(root)   # 第一次不进行选择直接扩展
        board_1 = copy.deepcopy(self.board)
        board_1._flip(expand_node.candidate, expand_node.color)
        ...
        if expand_node.color == 'X':
            ...
            self.simulation(expand_node)   
        expand_node.next_locations = list(expand_node.board.get_legal_actions())
        expand_node.board.pieces_index()
        #if (expand_node.board.black_count+expand_node.board.white_count) == 64:
        if expand_node.board.terminate():
            X_win = False
            if expand_node.board.black_count > expand_node.board.white_count:
                X_win = True


            if root.color == 'X' and X_win:
                expand_node.score = 1
            elif root.color == 'O' and not X_win:
                expand_node.score = 1
        ...
        while i < self.r and len(expand_node.next_locations) != 0:
            ...
            board_2._flip(expand_node.candidate)
        ...
        for child in root.child:
            coord = coord_to_chess_notation(child.candidate[0], child.candidate[1])
            print(
                f'If crazy AI action is {coord}, the weight will be {100 * mcts_prob[child.candidate[0]][child.candidate[1]]:.2f}%.')


        print(
            f'The best action is {coord_to_chess_notation(action[0],action[1])}, the weight will be {100 * mcts_prob[action[0]][action[1]]:.2f}%.')
        
        return action, mcts_prob

def coord_to_chess_notation(x, y):
    """
    将坐标从数字格式转换为黑白棋的标准格式（例如 "B4"）。
    """
    return chr(65 + y) + str(x + 1)